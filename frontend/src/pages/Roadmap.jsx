    ...
    <div className="relative">
      {loading && (
        <div className="absolute inset-0 flex flex-col items-center justify-center py-20 z-10">
          ...
          <p className="text-xs text-slate-400 mt-1">
            Interactive roadmap from roadmap.sh
          </p>
        </div>
      )}
      <div
      ...
      <div className="mt-3 flex items-center justify-between neu-in rounded-xl px-4 py-2.5">
        <span className="text-xs text-slate-400">
          Powered by roadmap.sh — interactive learning paths
        </span>
        <NeuButton
...
function ProgressTracker() {
  const total = ROADMAP_STAGES.reduce((s, st) => s + st.nodes.length, 0);
  const done = ROADMAP_STAGES.reduce(
    (s, st) => s + st.nodes.filter((n) => n.status === "done").length,
    0,
  );
  const active = ROADMAP_STAGES.reduce(
            ...
            >
              <NeuCard className="relative overflow-hidden">
                ...
                <div className="flex items-center gap-3 mb-3">
                  <div
                    className={`w-10 h-10 rounded-xl ${stage.gradient} flex items-center justify-center flex-shrink-0`}
                  >
  ...
  return (
    <div className="p-4 md:p-5 pb-24 md:pb-6">
      <SectionHeader
        ...
        sub="Track your progress · Explore interactive roadmap.sh paths"
      />
