...
function CertCard({ cert }) {
  ...
  return (
    <div
      className={`${grad} rounded-2xl p-4 text-white relative overflow-hidden`}
    >
    ...
    >
      <NeuCard>
        <div className="flex items-start justify-between mb-2">
          <div className="font-bold text-sm text-slate-700 dark:text-slate-200 flex-1 mr-2">
            {project.name}
          </div>
        ...
        <div className="flex flex-wrap gap-1.5">
          {(project.tech || []).map((t, ti) => (
            <span
              key={ti}
              className="text-[9px] font-bold neu-in px-2 py-0.5 rounded-md text-violet-500"
            >
              {t}
            </span>
          ))}
...
function EditModal({ user, onSave, onClose }) {
  ...
  const fields = [
    { key: "name", label: "Display Name", placeholder: "Jane Developer" },
    {
      key: "bio",
      label: "Bio",
      placeholder: "Full-Stack Developer in training…",
    },
    {
      key: "github",
      label: "GitHub URL",
...
export default function Profile() {
  ...
  const overallProgress = Math.round(
    PRACTICE_DOMAINS.reduce((s, d) => s + getDomainPercentage(d.id), 0) /
  ...
  return (
    <div className="p-4 md:p-5 pb-24 md:pb-6">
      ...
      <SectionHeader
        title="My Profile"
        sub="Track your progress, certifications & projects"
      />
      ...
      >
        <NeuCard className="mb-4">
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4">
            ...
            <div className="flex-1 text-center sm:text-left">
              ...
              <div className="flex flex-wrap gap-2 justify-center sm:justify-start">
                ...
                <NeuButton
                  onClick={() => setEditing(true)}
                  className="py-1.5! text-[11px]! px-3!gap-1.5"
                >
          ...
          <div className="grid grid-cols-4 gap-2 mt-4">
            {[
              {
                val: user.problemsSolved || 0,
                label: "Problems",
                color: "text-grad-purple",
                icon: Code2,
              },
              {
                val: user.totalHours || 0,
                label: "Hours",
                color: "text-grad-amber",
                icon: Clock,
              },
              {
                val: `${user.streak || 0}🔥`,
                label: "Streak",
                color: "text-grad-green",
                icon: Flame,
              },
              {
                val: certs.length,
                label: "Certs",
                color: "text-grad-teal",
                icon: Award,
              },
            ].map(({ val, label, color }) => (
          ...
          {PRACTICE_DOMAINS.map((d, i) => {
            ...
            return (
              ...
              >
                ...
                >
                  <div
                    className={`w-9 h-9 rounded-xl ${d.gradient} flex items-center justify-center shrink-0`}
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex justify-between items-center mb-1">
                      ...
                      <span className={`text-xs font-bold ${d.textGrad}`}>
                        {pct}%
                      </span>
                    </div>
          ...
          <NeuCard className="mb-4">
            ...
            <div className="flex flex-wrap gap-2">
              {user.skills.map((skill, i) => (
                <span
                  key={i}
                  className="neu-in px-3 py-1.5 rounded-xl text-xs font-semibold text-violet-500"
                >
                  {skill}
                </span>
              ))}
        ...
        {certs.length > 0 ? (
          ...
          <NeuCard className="mb-4 text-center py-8">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-3">
              Complete a full domain to earn your first certificate!
            </p>
            <NeuButton
              variant="primary"
      ...
      >
        <div className="flex items-center justify-between mb-3">
          ...
          <NeuButton
            onClick={() => setEditing(true)}
            className="py-1!text-[11px]! px-2.5! gap-1"
          >
            <Plus size={11} /> Add
          </NeuButton>
        </div>
        {user.projects?.length > 0 ? (
          ...
          <NeuCard className="mb-4 text-center py-6">
            ...
            <p className="text-sm text-slate-400 mb-3">
              Showcase your GitHub projects here
            </p>
            <NeuButton
              onClick={() => setEditing(true)}
