...
import {
  Upload, FileText, Download, Bot, CheckCircle2,
  AlertCircle, XCircle, Loader2, Sparkles, Target,
  Key, Lightbulb, FileDown,
} from 'lucide-react'
...
function DropZone({ file, onFile, dragging, setDragging }) {
  ...
  const handleDrop = useCallback((e) => {
    ...
    if (f && (ACCEPTED.includes(f.type) || f.name.endsWith('.pdf') || f.name.endsWith('.docx'))) {
      onFile(f)
    }
  }, [onFile, setDragging])
    ...
    >
      ...
      {file ? (
        <>
          ...
          <p className="text-sm font-semibold text-emerald-600 dark:text-emerald-400">{file.name}</p>
...
function ScoreRing({ score }) {
  const color = score >= 80 ? 'text-grad-green' : score >= 60 ? 'text-grad-amber' : 'text-grad-pink'
  return (
    <div className="neu-in rounded-2xl p-6 text-center">
      <div className={`text-5xl font-extrabold font-display ${color}`}>{score}</div>
      <div className="text-xs text-slate-400 font-semibold mt-1">ATS Score</div>
      <ProgressBar value={score} color={score >= 80 ? 'gradient-green' : score >= 60 ? 'gradient-amber' : 'gradient-pink'} className="mt-3" />
    </div>
  )
}
...
export default function Resume() {
  ...
  const handleOptimize = async () => {
    ...
    try {
      ...
      setError(err.message || 'Optimization failed. Ensure the backend is running and ANTHROPIC_API_KEY is set.')
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}>
          <NeuCard hover={false}>
            ...
            />
            <p className="text-[10px] text-slate-400 mt-2">{jd.length} chars · Minimum 20 required</p>
          </NeuCard>
        </motion.div>
      ...
      >
        ...
        {!canOptimize && !loading && (
          <p className="text-center text-xs text-slate-400 mt-2">
            {!jd.trim() ? 'Add a job description to continue' : !file ? 'Upload your resume to continue' : ''}
          </p>
        )}
      </motion.div>
            ...
            {activeTab === 'analysis' && (
              <div className="space-y-4">
                ...
                {result.suggestions?.length > 0 && (
                  <NeuCard hover={false}>
                    <h4 className="font-bold text-sm text-slate-700 dark:text-slate-200 flex items-center gap-2 mb-3">
                      <Lightbulb size={14} className="text-amber-500" /> AI Suggestions
                    </h4>
                    <div className="space-y-2">
                      {result.suggestions.map((s, i) => (
                        <div key={i} className="flex items-start gap-2.5 neu-in rounded-xl px-3 py-2.5">
                          {s.type === 'critical' ? (
                ...
                <NeuCard hover={false}>
                  <h4 className="font-bold text-sm text-slate-700 dark:text-slate-200 flex items-center gap-2 mb-3">
                    <FileDown size={14} className="text-violet-500" /> Download Optimized Resume
                  </h4>
              ...
              <NeuCard hover={false}>
                <div className="flex items-center justify-between mb-4">
                  <h4 className="font-bold text-sm text-slate-700 dark:text-slate-200 flex items-center gap-2">
                    <Bot size={14} className="text-violet-500" /> Optimized Resume
                  </h4>
