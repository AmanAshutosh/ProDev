...
const buildDefault = () =>
  Object.fromEntries(
    PRACTICE_DOMAINS.map((d) => [
      d.id,
      { percentage: 0, topics: {}, hoursSpent: 0 },
    ]),
  );
...
export function ProgressProvider({ children }) {
        ...
        data.forEach((p) => {
          map[p.domain] = {
            ...
            topics: Object.fromEntries(
              (p.topics || []).map((t) => [t.id, t.completed]),
            ),
  ...
  const getTopicStatus = (domainId, topicId) =>
    !!progress[domainId]?.topics[topicId];
